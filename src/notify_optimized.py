                ...
                if members and 'data' in members:
                    ...
                    if 'members' in data:
                        ...
                        if isinstance(members_list, list):
                            ...
                            print(f"   成員 ID: {list(member_table.keys())}")
                            ErrorHandler.log_operation("GQL members processing", {"processed_count": len(member_table)})
                        else:
                            print(f"❌ 成員列表不是列表類型: {type(members_list)}")
                    else:
                        print(f"❌ GQL 響應中沒有 'members' 字段")
                        print(f"   可用字段: {list(data.keys())}")
                else:
                    print(f"❌ GQL 響應格式錯誤")
                    if members:
                        print(f"   響應結構: {list(members.keys())}")
                    else:
                        print(f"   響應為 None")
        else:
            member_table = {}
