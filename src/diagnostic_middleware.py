...
class DiagnosticMiddleware:
    ...
    async def run_full_diagnostic() -> Dict[str, Any]:
        ...
        try:
            ...
            if diagnostic_result["notifications_test"] and diagnostic_result["notifications_test"]["status"] != "success":
                issues.append(f"通知測試失敗: {diagnostic_result['notifications_test'].get('error', '未知錯誤')}")
            
            if diagnostic_result["socialpage_test"] and diagnostic_result["socialpage_test"]["status"] != "success":
                issues.append(f"社交頁面測試失敗: {diagnostic_result['socialpage_test'].get('error', '未知錯誤')}")
            
            diagnostic_result["summary"]["issues"] = issues
            diagnostic_result["summary"]["overall_status"] = "healthy" if not issues else "unhealthy"
